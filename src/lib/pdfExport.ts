...
function generatePDFHTML(
  ...
  // تصميم صفوف الجدول بمسافات (Padding) متناسقة وتوسيط دقيق
  const tableRows = sortedTxns
    .map((tx, i) => `
      <tr style="background: ${i % 2 === 0 ? '#fcf9f2' : '#ffffff'};">
        <td style="padding: 12px 10px; text-align: center; border-bottom: 1px solid #e5ddd0; font-size: 13px;">${tx.date}</td>
        ...
        <td style="padding: 12px 10px; text-align: right; border-bottom: 1px solid #e5ddd0; line-height: 1.5; font-size: 13px;">${tx.details}</td>
        <td style="padding: 12px 10px; text-align: center; border-bottom: 1px solid #e5ddd0; font-weight: 700; font-size: 14px;">${formatNumberEn(Math.abs(tx.balance))}</td>
      </tr>
    `)
    .join('');

  return `
    <div id="pdf-content" style="direction: rtl; font-family: 'Segoe UI', Tahoma, sans-serif; padding: 30px; width: 190mm; margin: 0 auto; background: white; color: #3c2814; box-sizing: border-box;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px; border-bottom: 3px solid #4a341c; padding-bottom: 20px;">
        ...
        <div style="flex: 1; padding-top: 10px;">
          ...
          <div style="font-size: 14px; color: #555; font-weight: 600; line-height: 1.8;">
            ${shopPhone ? `<div>📞 الهاتف: <span style="direction: ltr; display: inline-block; font-weight: bold; color: #333;">${shopPhone}</span></div>` : ''}
            ${shopAddress ? `<div>📍 العنوان: <span style="color: #333;">${shopAddress}</span></div>` : ''}
          </div>
        </div>
        ...
        <div style="text-align: left; flex: 1; display: flex; flex-direction: column; align-items: flex-end;">
          ...
          <div style="font-size: 12px; color: #888; font-weight: bold;">تاريخ الإصدار: ${exportDate}</div>
        </div>
      </div>
      ...
      <div style="display: flex; gap: 20px; margin-bottom: 30px;">
        <div style="flex: 1; background: #fff1f2; padding: 20px 15px; border-radius: 10px; text-align: center; border: 1px solid #fecdd3;">
          <div style="font-size: 13px; color: #be123c; font-weight: 800; margin-bottom: 8px;">إجمالي عليه</div>
        ...
        <div style="flex: 1; background: #f0fdf4; padding: 20px 15px; border-radius: 10px; text-align: center; border: 1px solid #bbf7d0;">
          <div style="font-size: 13px; color: #15803d; font-weight: 800; margin-bottom: 8px;">إجمالي له</div>
        ...
        <div style="flex: 1; background: #fffbeb; padding: 20px 15px; border-radius: 10px; text-align: center; border: 1px solid #fde68a;">
          <div style="font-size: 13px; color: #b45309; font-weight: 800; margin-bottom: 8px;">الرصيد النهائي</div>
          <div style="font-size: 22px; font-weight: 900; color: #b45309;">
            ${formatNumberEn(Math.abs(netBalance))} <span style="font-size: 16px;">${netBalance >= 0 ? 'عليه' : 'له'}</span>
          </div>
        </div>
      </div>
      ...
      <div style="margin-top: 40px; padding-top: 15px; border-top: 1px solid #e5ddd0; text-align: center;">
        <p style="font-size: 13px; color: #666; font-weight: 700; margin: 0;">
