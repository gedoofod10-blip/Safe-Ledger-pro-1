...
const MainMenu = () => {
  ...
  const menuItems = [
    { icon: UserPlus, label: 'إضافة عميل', color: 'text-blue-600', bg: 'bg-blue-50 border-blue-100', action: () => navigate('/clients') },
    { icon: Plus, label: 'إضافة مبلغ', color: 'text-green-600', bg: 'bg-green-50 border-green-100', action: () => navigate('/add-transaction') },
    { icon: FileText, label: 'التقارير الشاملة', color: 'text-purple-600', bg: 'bg-purple-50 border-purple-100', action: () => navigate('/reports') },
    { icon: Settings, label: 'الإعدادات', color: 'text-gray-600', bg: 'bg-gray-50 border-gray-200', action: () => navigate('/settings') },
    { icon: Share2, label: 'مشاركة التطبيق', color: 'text-cyan-600', bg: 'bg-cyan-50 border-cyan-100', action: handleShare },
    { icon: Info, label: 'حول البرنامج', color: 'text-amber-600', bg: 'bg-amber-50 border-amber-100', action: () => setShowAbout(true) },
    { icon: LogOut, label: 'خروج', color: 'text-red-600', bg: 'bg-red-50 border-red-100', action: () => navigate('/clients') },
  ];
